...
data class WifiEntry(var title: String = "", var password: String = "", var type: Type = Type.NONE) {
    enum class Type {
        WPA {
            override fun asQRCodeAuth() = "WPA"
        },
        WEP {
            override fun asQRCodeAuth() = "WEP"
        },
        NONE {
            override fun asQRCodeAuth() = "nopass"
        },
        ENTERPRISE {
            override fun asQRCodeAuth(): String {
                throw GetEAPTypeException()
            }
        };

        abstract fun asQRCodeAuth(): String
...
private fun readNetworkList(parser: XmlPullParser): ArrayList<WifiEntry> {
    ...
    while (doLoop) {
        try {
            ...
            e.message?.let { Log.e("LoadData.NetworkList", it) }
            doLoop = false
        }
